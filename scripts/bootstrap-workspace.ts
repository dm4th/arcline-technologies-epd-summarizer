...
function writeNotionIds(dbIds: Record<DbKey, string>, pageIds: Record<PageKey, string>): void {
  ...
  const ownedKeys = new Set(Object.keys(dbIds));

  // Read-merge: collect dbs.* lines from the existing file that are NOT owned by
  // this function (e.g. hitlReviewSessions from PRD-06, GTM fields from patch-schema-round2.ts).
  // Preceding comment lines are bundled with the field they document.
  const collectExtra = (block: string, fieldRe: RegExp): string => {
  ...
  let extraInterfaceLines = "";
  let extraConstLines = "";

  if (fs.existsSync(outPath)) {
    const existing = fs.readFileSync(outPath, "utf8");
    // Two sequential exec() calls on a shared /g regex: first hit = interface dbs block,
    // second hit = NOTION_IDS dbs block (they appear in that order in the file).
    const dbsRe = /dbs:\s*\{([^}]+)\}/g;
    const ifaceMatch = dbsRe.exec(existing);
    const constMatch = dbsRe.exec(existing);
    if (ifaceMatch) extraInterfaceLines = collectExtra(ifaceMatch[1], /^\s*(\w+):\s*string/);
    if (constMatch) extraConstLines     = collectExtra(constMatch[1], /^\s*(\w+):\s*"[^"]+"/);
  }

  const ifaceExtra = extraInterfaceLines ? "\n" + extraInterfaceLines : "";
  const constExtra = extraConstLines     ? "\n" + extraConstLines     : "";
...
export interface NotionIds {
  dbs: {
    ...
    agentRunLog: string;
    deliveryPipeline: string;${ifaceExtra}
  };
  pages: {
    squadAtlas: string;
    squadLumen: string;
...
export const NOTION_IDS: NotionIds = {
  dbs: {
    ...
    deliveryPipeline:   "${dbIds.deliveryPipeline}",${constExtra}
  },
  pages: {
